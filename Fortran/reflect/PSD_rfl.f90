...
Program main
    ...
    continue
        ...
        64 format(15X,'Segment Index',15X,'Hitting Probability')
    ...
    open(unit=36,file=TRIM(rue_total)//"debug_rfl.txt"); write(36,94) date(1), date(2), date(3), date(5), date(6); write(36,*)
    ...
    do i = 1, num_particle
        ...
        if (i.le.20) write(32,*) "# ",i !! Furnish particle index in traj.
        position(i,:) = 0.0d0; g_act = 1
        if (i.le.20) write(32,72) position(i,1), position(i,2), g_act, 0.0d0, time_diff, ell
        ...
        if (i.le.20) write(32,72) position(i,1), position(i,2), g_act, radius_min, time_diff, ell
        ...
        do while (g_act < Generation_max + 3) !! 等价于 (1.gt.0)
            ...
            !if (radius_min > 0.0d0) then
                call diffusion(position(i,1), position(i,2), radius_min)
                time_diff = time_diff + radius_min**2 / (4.0d0 * diff_coef)
                if (i.le.20) write(32,72) position(i,1), position(i,2), g_act, radius_min, time_diff, ell !! Record trajs of each particle
                ...
                if (g_act == Generation_max+2) then
                    ...
                    do while (inside == 1) !! Repete the process once the particle enter the layer.
                        ...
                        &coord_x(noloop(k+2)),coord_y(noloop(k+2)), cross, inside, touch, radius_min, time_diff, ell)
                        ! if (cross.ne.0) then test if k change? cross = ±1, k = k + cross
                        if (abs(cross) == 1) k = k + cross
                    ...
                    if (ell >= ell_hat) then
                        ...
                        if (i < 50) write(33,73) position(i,1), position(i,2), time_diff, ell, ell_hat !! Print final positions, diffusion time, local time
...
subroutine cherche(niveau, connu1, connu2, gap, nouveau1, nouveau2, combien)
    ...
    implicit none !! "niveau" = g_act/g_test;
    include "para.h" !! "gap" = distance of two point indices
    integer,intent(in) :: niveau, connu1, connu2, gap !! "connu1/2" refer to obvious search range, namely #rec(g_act-1,:)
...
subroutine reflection(x1,y1, x2,y2, x0,y0, x3,y3, x4,y4, croise, interieure, touche, rayon, time, local_time)
    ...
    integer :: case1, case2, case_new, case_angle !! Left/Right boundary point environments. Condition after diffusion.
    real*8 :: d1, d2, radius_min, d14, d23 !! Left/Right boundary point distance. 
    real*8 :: deltax, deltay, dist_xy, theta, st, ct, xdist, ydist, dxnew, dynew !! Variables for Rotations.
    ...
    if (croise == 0) then !! Assure that even after the diffusion, the particle doesn't enter other boundary. septds3!
        ...
        if (case1 > case2) then !! Particle closer to right boundary point
            call distance(x2,y2, x4,y4, x1 + ct * dxnew - st * dynew, y1 + st * dxnew + ct * dynew, case_new, d2)
            ...
            call distance(x3,y3, x1,y1, x1 + ct * dxnew - st * dynew, y1 + st * dxnew + ct * dynew, case_new, d1)
            select case (case_new)
                case(0) !! angle = 120
                    if (dynew > eps) then
                        interieure = 0
