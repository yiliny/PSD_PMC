Program main
    ...
    rue_total = "diff-"
    open(unit=31,file=TRIM(rue_total)//"coord_v6.txt"); write(31,94) date(1), date(2), date(3), date(5), date(6)
    ...
    open(unit=36,file=TRIM(rue_total)//"debug_v6.txt"); write(36,94) date(1), date(2), date(3), date(5), date(6)
    ...
    do i = 1, num_particle
        ...
        1101 continue
        !write(36,*) "Done for first order.",i
        !write(*,*) "Done for first order.",i
        if (i <= 50) write(32,*) position(i,1), position(i,2), g_act !! Record trajs of each particle
