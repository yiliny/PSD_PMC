...
Program main
    ...
    integer :: qmax, g_last, g_test, case, infvl, supvl, search_length, k1, k2!, kint
    real*8 :: angle_ini, tta, new_x, new_y, radius_min, radius, u, l1, l2!,lint
    ...
    do i = 1, num_particle
        ...
        position(i,:) = 0.0d0; g_last = 1
        if (i.le.50) write(32,*) position(i,1), position(i,2), g_last
		...
		!### [1.1] **Initialize `g_last` ** (for start-up)
        g_last = 2 !! g_last = {1, 2, 3, ..., Gmax+2} ~ g = {0, 0, 1, ..., Gmax}
		...
		!! Iteration for GAFRW
        do while (g_last < Generation_max + 3) !! 等价于 (1.gt.0)
            ...
            continue
                ...
                do j = 1, search_length !! 为何放弃 do j = infvl, supvl, int_gap? 答曰考虑区间跨过首尾分界点不严格递增
                    ...
                    !write(36,*) "[2] g_last =", g_last, "search_index & span =",k,u
            ...
            search_length = noloop(rec_index(g_last,2) - rec_index(g_last,1) + num_points - 1) / int_gap + 1
            !write(36,*) "[3]: #rec(g_last,:) =", rec_index(g_last,1), rec_index(g_last,2), "search length =", search_length
            allocate(search_index(search_length))
            do j = 1, search_length
                search_index(j) = noloop(rec_index(g_last,1) + (j-1) * int_gap)
            end do
            ...
            if (radius_min > 0.0d0) then
                call diffusion(position(i,1), position(i,2), radius_min)
                if (i.le.50) write(32,*) position(i,1), position(i,2), g_last !! Record trajs of each particle
                !write(36,*) "Diffusion->", position(i,1), position(i,2)
            else !! radius_min == 0.0d0
                if (g_last == Generation_max+2) then
                    if (i.le.50) write(32,*) position(i,1), position(i,2), g_last !! Print final positions
                    ...
                    g_last = g_last + 1
                    !goto 1103 !! Jump the g_test loop.
                end if
            end if


            !do j = 2, g_last !Generation_max+2
            ...
            do g_test = 2, g_last!-1 !! Search all passed level g. Make sure the particle not out of region at all level.
                ...
                & (noloop(rec_index(g_test,2) - rec_old2 + num_points - 1) > 0)) then
                    ...
                    if (g_last > Generation_max+2) g_last = Generation_max
                    !goto 1103
                else 
                    !! 范围可能缩小，于是重新赋值
            ...
            !if ((rec_index(g_last,2) - rec_index(g_last,1) == int_gap) .or. &
            !& noloop(rec_index(g_last,2) - rec_index(g_last,1) + num_points - 1) == int_gap) then
            if (noloop(rec_index(g_last,2) - rec_index(g_last,1) + num_points - 1) == int_gap) then
                if (g_last < Generation_max + 2) g_last = g_last + 1
            end if
