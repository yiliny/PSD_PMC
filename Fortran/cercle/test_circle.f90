...
Program main
    ...
    continue !! formats
        ...
        94 format('Circle Test @ YYE. Version 1.1.1.. Simulation on ',I4,2('-',I2),'   ',I2,'h',I2)
    ...
    num_particle = 1000000
    traj_max = 5; final_max = 100
    ...
    rho_ratio = 2.0d0; diff_coef = 1.0d0 !! Determine diffusion/reflection parameters
    taille = 1.0d0
    eps = 1.0d-3
    q_rect = 1.0d1
    ...
    open(unit=35,file=TRIM(rue_total)//"pbana.txt")
        ...
        write(35,*) "# r0, θ0 = ", r0, theta0; write(35,*); write(34,*) "# r0, θ0 = ", r0, theta0; write(34,*)
    ...
    do i = 1, num_particle !! Diffusion for all particles
        ...
        if (i <= final_max) then
            ...
            write(32,*) "Local Time Threshold =", ell_hat; write(32,*)
        end if
    ...
    deallocate(position); deallocate(coord_x); deallocate(coord_y); deallocate(account); close(31)
